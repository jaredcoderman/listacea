...
const List: React.FC<Props> = (props) => {
  ...
  let draggedItemId = "" // Classname
  let dropCategoryId = ""
  const dragStart: DragEventHandler<HTMLDivElement> = (e) => {
    setDragged(e.target)
  }

  const dragDrop: DragEventHandler<HTMLDivElement> = async (e) => {
    let target = e.target as HTMLDivElement | HTMLHeadingElement | HTMLSpanElement | null
    while (target && target.className !== 'can-drop-into') {
      target = target.parentNode as HTMLDivElement;
    }
    let category = target.id
    await fetch(`/api/v1/item/${dragged.id}`, {
      method: "PATCH",
      headers: { 'Content-Type': 'application/json' },
  ...
  for(const [category, itemList] of Object.entries(items)) {
    let mappedNotPurchased = itemList.filter(item => item.purchased == false).map((item: ItemProps) => {
    ...
    let element = <div id={category} onDrop={(event: React.DragEvent<HTMLDivElement>) => dragDrop(event)} onDragOver={(event) => event.preventDefault()} className="can-drop-into" key={itemsWithCategories.length}>
      <h2>{category}</h2>
      {mappedNotPurchased}
      {mappedPurchased}
    </div>
